...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Pega o valor do parâmetro 'login'
    ...
    if (loginValue == 1) {
        ...
        const allMenuLinks = document.querySelectorAll('.slicknav_nav a, .header__menu a');
        allMenuLinks.forEach(link => {
            const href = link.getAttribute('href');
            if (href && !href.includes('login=1')) {
                const newHref = href.includes('?') ? `${href}&login=1` : `${href}?login=1`;
